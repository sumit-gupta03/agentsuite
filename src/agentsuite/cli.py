...
::
    ...
    agentsuite de "profile raw.orders"              one-shot against a domain
    agentsuite snowflake "list the schemas"         a preset
    agentsuite chat --domain de                     interactive session
    agentsuite domains                              what is installed
    agentsuite skills list --domain de              what is loaded, and from where
    agentsuite skills show incremental-backfill
    agentsuite tools list --domain de
    agentsuite init                                 scaffold ./.agentlib
    agentsuite doctor --domain de                   the resolved session + system prompt
...
BANNER = "agentsuite -- type /exit to leave, /help for commands"

INIT_CONFIG = """\
# agentsuite project configuration
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="agentsuite", description="One core, many domains.")
...
def _build_agent(domain: str, args: argparse.Namespace) -> Any:
    import agentsuite as package

    factory = getattr(package, domain, None)
    if factory is None:
        raise ConfigError(f"unknown domain or preset {domain!r}. Try: agentsuite domains")
...
def _cmd_skills(argv: list[str]) -> int:
    ...
    if action == "show":
        if len(words) < 2:
            print("usage: agentsuite skills show <name>", file=sys.stderr)
            return 2
        from .core import domain as domain_module
