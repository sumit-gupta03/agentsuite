...
Requires boto3: ``pip install "agentsuite[bedrock]"``.
...
class BedrockModel:
    """A :class:`~agentsuite.core.model.Model` backed by Bedrock's Converse API.
    ...
    def __init__(
        ...
        **client_kwargs: Any,
        ...
        try:
            ...
            raise ModelError(
                'boto3 is not installed. Install it with: pip install "agentsuite[bedrock]"'
            ) from exc
        ...
        if not resolved_region:
            raise ModelError(
                ...
                "deep with an error that says nothing about agentsuite."
            )
        try:
