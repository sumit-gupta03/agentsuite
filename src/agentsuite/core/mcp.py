...
* **Every result is fenced and scanned** by :mod:`agentsuite.core.untrusted` before
...
Requires the ``mcp`` package: ``pip install "agentsuite[mcp]"``. The
    ...
    def _connect(self, server: MCPServer) -> Any:
        ...
        try:
            ...
            raise ConfigError(
                ...
                'Install it with: pip install "agentsuite[mcp]"'
            ) from exc
